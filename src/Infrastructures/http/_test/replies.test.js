...
describe('/threads/:threadId/comments/:commentId/replies endpoint', () => {
  ...
  describe('when POST /threads/:threadId/comments/:commentId/replies', () => {
    it('should response 201 and persisted reply', async () => {
      ...
      // Create thread and comment first
  ...
  describe('when DELETE /threads/:threadId/comments/:commentId/replies/:replyId', () => {
    it('should response 200 when reply deleted successfully', async () => {
      ...
      // Create thread, comment, and reply
      const threadResponse = await request(app)
        .post('/threads')
        .set('Authorization', `Bearer ${accessToken}`)
        .send({
    ...
    it('should response 404 when comment not found', async () => {
      ...
      // Action
      const response = await request(app)
        .delete(`/threads/${threadId}/comments/comment-xxx/replies/reply-123`)
        .set('Authorization', `Bearer ${accessToken}`);
    ...
    it('should response 403 when user is not the reply owner', async () => {
      ...
      // Create thread, comment, and reply with first user
      const threadResponse = await request(app)
        .post('/threads')
